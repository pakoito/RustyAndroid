...
void _Unwind_SetGR(struct _Unwind_Context* ctx, int index, uint64_t new_value) {
...
void _Unwind_SetIP(struct _Unwind_Context* ctx, uintptr_t new_value) {
  uint32_t val = (uint32_t)new_value;
  // Propagate thumb bit to instruction pointer
  uint32_t thumbState = _Unwind_GetGR(ctx, UNWIND_IP_REG) & 1;
  uint64_t new_val = (uint64_t)(val | thumbState);
  _Unwind_SetGR(ctx, UNWIND_IP_REG, new_val);
}
